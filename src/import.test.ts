import { Store, importFile, validateProject } from "./import";
...
function uniqueIds({ projects, labels }: Store) {
...
// different each time).
function stripIds({ title, projects, labels }: Store) {
  return {
    title,
...
function utcTimes({ title, projects, labels }: { title?: string; projects: any[]; labels: any[] }) {
  return {
    title,
...
describe("importFile", () => {
  ...
  it("imports projects with labels", () => {
    ...
    const projectOne = result.projects[0];
    const projectTwo = result.projects[1];
    expect(projectOne.labels && projectOne.labels.length).toBe(1);
    expect(projectTwo.labels && projectTwo.labels.length).toBe(2);
