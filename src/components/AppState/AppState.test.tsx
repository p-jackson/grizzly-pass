...
function renderAppState(file = jsonFile) {
  ...
  return shallow(<AppState db={() => null as any} readFileAsText={readFileAsText} />);
}
...
it("has no error message by default", () => {
  expect(appState.state("errorMessage")).toBeUndefined();
});
...
it("defaults to no tabs selected", () => {
  ...
  expect(appState.find(App).prop("selectedTab")).toBeUndefined();
});
...
it("updates the selectedTab state", () => {
  ...
  (appState.instance() as AppState).handleTabChange(tabIds[0]);
  expect(appState.find(App).prop("selectedTab")).toBe(tabIds[0]);
});

it("updates the projects state if the dropped file is json", async function () {
  const mockFile = { MOCK: "FILE" };
  await (appState.instance() as AppState).handleFileDrop(mockFile as any);
  expect(readFileAsText).toHaveBeenCalledWith(mockFile);
  expect(appState.state("errorMessage")).toBeUndefined();
  expect(appState.state("title")).toBeUndefined();
